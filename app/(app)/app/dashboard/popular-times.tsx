...
/** Map a count value to a busyness tier based on its ratio to the global max. */
function getBusynessLabel(count: number, globalMax: number): string {
...
export function PopularTimes({ data }: { data: DayData[] }) {
  ...
  const currentLabel = currentHourData
    ? getBusynessLabel(currentHourData.avg, globalMax)
    : "";
    ...
    <div className="space-y-4" data-testid="popular-times">
        ...
        <div className="flex gap-1">
          {DAY_LABELS.map((label, idx) => (
            ...
            >
              ...
              <span className="sm:hidden">{SHORT_DAY_LABELS[idx]}</span>
            </button>
          ))}
        </div>

        {/* Busyness indicator for current hour */}
        {selectedDow === todayDow && currentLabel && (
          <span
      ...
      >
        {visibleHours.map((h) => {
          ...
          const label = getBusynessLabel(h.avg, globalMax);
          ...
          return (
            ...
            >
              ...
              <div className="pointer-events-none absolute -top-10 left-1/2 z-10 hidden -translate-x-1/2 rounded bg-popover px-2 py-1 text-[10px] font-medium text-popover-foreground shadow-md group-hover:block whitespace-nowrap">
                {label || "No visitors"} · ~{h.avg} visitor{h.avg !== 1 ? "s" : ""}
              </div>
      ...
      <div className="flex flex-wrap items-center gap-3 text-[10px] text-muted-foreground sm:text-xs">
        <span className="flex items-center gap-1">
          <span className="inline-block size-2.5 rounded-sm bg-green-400/60" /> Free
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block size-2.5 rounded-sm bg-blue-400/60" /> Comfortable
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block size-2.5 rounded-sm bg-orange-400/70" /> Moderate
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block size-2.5 rounded-sm bg-red-500/70" /> Busy
        </span>
      </div>
    </div>
  );
